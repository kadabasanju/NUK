...
/*public ArrayList<Cluster> getClusterListWithNoData(ArrayList<Cluster> clu, long endTime){
   ...
   /*	Cluster temp1,next1;
   	for(int i =0; i<cluLen-1; i++){
   		...
   		long du;
   		if(i<cluLen-2){
   		du = (Long.parseLong(next1.getTime()) - Long.parseLong(temp1.getTime()))/1000;}
   		else
   		{
   		du = (System.currentTimeMillis()-Long.parseLong(next1.getTime()))/1000;	
   		}
   		if(du>600){
   			errorCluster.add(temp1);
   			//Log.i(getReadableTime(Long.parseLong(temp1.getTime())), getReadableTime(Long.parseLong(next1.getTime())));
   	...
   	/*for (int i = 0; i < cluLen; i++ )
   	{
   		...
   		Cluster next = clu.get(i+1);
   		long timeDiff = Long.parseLong(next.getTime()) - Long.parseLong(temp.getTime());
   		...
   		if(timeDiff>900000){
   			...
   			Cluster noDataCluster = new Cluster(temp.getLatitude(), temp.getLongitude(), temp.getTime(), timeDiff,temp.getAccuracy(),"NoData",temp.getProvider(), temp.getSpeed(), 0);
   			resultCluster.add(temp);
   			resultCluster.add(noDataCluster);
   			temp = next;
   			continue;
   		...
   		{	double latAvg1=0, longAvg1=0, accuracy1 =0, altitude = 0;
			...
			altitude = Math.max(temp.getAltitude(), next.getAltitude());
   			...
   			//double alti = Math.max(temp.getAltitude(), next.getAltitude());
			Cluster avgCluster = new Cluster(latAvg1, longAvg1, temp.getTime(), temp.getDuration(), accuracy1, "activity",temp.getProvider(), temp.getSpeed(), altitude );
			
   			tempArray.add(temp);
   			temp = avgCluster;
   			if(i==clu.size()-2)
       		{
       			long du = endTime - Long.parseLong(tempArray.get(0).getTime());
       			tempArray.add(next);	
				double lat = 0, longi = 0, accu = 0, alti=0;
				for(int a =0; a < tempArray.size(); a++){
					lat += tempArray.get(a).getLatitude();
				...
				Cluster avgClu = new Cluster(lat, longi, tempArray.get(0).getTime(), du/1000, accu, "activity" ,temp.getProvider(), temp.getSpeed(), alti);
				tempArray = new ArrayList<Cluster>();
   			resultCluster.add(avgClu);
       		}
   			...
   			if(tempArray.size() == 0){
   			long dura;
				dura = Long.parseLong(next.getTime()) - Long.parseLong(temp.getTime());
				
   			temp.setDuration(dura/1000);
   			resultCluster.add(temp); temp = next;}
   			else{
   				tempArray.add(temp);
				long dura = (Long.parseLong(next.getTime()) - Long.parseLong(tempArray.get(0).getTime()));
				double latAvg = 0, longAvg = 0, accuracy = 0, alti = 0;
				for(int a =0; a < tempArray.size(); a++){
					latAvg += tempArray.get(a).getLatitude();
					longAvg += tempArray.get(a).getLongitude();
				...
				Cluster avgCluster = new Cluster(latAvg, longAvg, tempArray.get(0).getTime(), dura/1000, accuracy, "activity",temp.getProvider(), temp.getSpeed(), alti );
				tempArray = new ArrayList<Cluster>();
   			resultCluster.add(avgCluster);
   			temp = next;
   			}
   			...
   			if(i==clu.size()-2)
       		{
       			long duration = endTime - Long.parseLong(next.getTime());
       			next.setDuration(duration/1000);
       			resultCluster.add(next);
       		}
   	...
   	for(int i =0; i<resLenNodes; i++){
   		Cluster node = finalClusters.get(i);
   		if(node.getActivity().equals("NoData")){
   			interimNodes.add(node);
   		}
   		else if(node.getDuration() > maxDuration ) 
   		{
   			if(i==0){ interimNodes.add(node);}
   			else
   			{	if(!(node.getTime().equals(finalClusters.get(i-1).getTime())))
   					interimNodes.add(node);
   			}
  ...
  //Removing faulty nodes.
  	if(interimNodes.size()>0)
   	...
   	for(int i =0; i<interimNodes.size()-1; i++){
   		if(interimNodes.get(i).getActivity().equals("NoData")){
   			resultNodes.add(interimNodes.get(i));
   		}
   		else{
   		nextInt = interimNodes.get(i+1);
   		long du1 = Long.parseLong(tempInt.getTime()) + tempInt.getDuration()*1000;
   		long du2 = Long.parseLong(nextInt.getTime());
   		if((du2-du1)/1000 < 900 ) 
   		{
	   		...
	   		if(dist < (tempInt.getAccuracy()+nextInt.getAccuracy()) && dist < 250)
	   		{
	   			long diff = Long.parseLong(nextInt.getTime())+ nextInt.getDuration()*1000;
	   			long du = (diff-Long.parseLong(tempInt.getTime()))/1000;
	   			tempInt.setDuration(du);
	   			Log.i(tempInt.getTime(), i+"dist: "+dist);
	   		}
	   		else{
